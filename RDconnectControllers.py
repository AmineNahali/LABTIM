...
def RDconnectApp():
    ...
    if session.get(f"{theuser},RDConnect") or session.get("su"):
        return render_template('RDConnect/RDConnect.html')
    else:
        return redirect("/loginUsers?initApp=RDConnect")

def RDconnectUserDetails():
    data = request.get_json(force=True)
    varMail = str(data["tr"])
    if session.get(f"{varMail},RDConnect") or session.get("su"):
        if global_col_users.count_documents({"Mail":varMail}) == 0:
            ...
            _usernameRD = str(tmpUSR["Nom"]) + " " +str(tmpUSR["Prenom"])
            _specRD = str(tmpUSR["Spec"])
            return jsonify({"msg":"success","user":str(_usernameRD),"spec":str(_specRD)})
    else:
        return jsonify({"msg":"redirected","urlto":"/loginUsers?initApp=RDConnect"})

def RDconnectLogOut():
    data = request.get_json(force=True)
    varMail = str(data["tr"])
    session[f"{varMail},RDConnect"] = None
...
def RDconnectAddPat():
    ...
    varMail = str(data["by"])
    dat = str(data["when"])
    if session.get(f"{varMail},RDConnect"):
        if global_col_patients.count_documents({"ID":npat}) == 0:
            ...
            global_col_patients.insert_one({"ID":npat,"App":"RDConnect","Ajout":dat,"Responsable":varMail})
            ##x = global_col_apps.find_one({"Nom_App":"RDConnect"})
            ##total_old = int(str(x["Total_patients"]))
            ##total_old = total_old+1
            ##global_col_apps.update_one({"Nom_App":"RDConnect"},{"$set":{"Total_patients":str(total_old)}})
            return jsonify({"msg":"success"})
        else:
        ...
        if global_col_patients.count_documents({"ID":npat}) == 0:
            ...
            os.makedirs("./upload/"+npat+"/tmp")
            os.makedirs("./upload/"+npat+"/tmp2")
            global_col_patients.insert_one({"ID":npat,"App":"RDConnect","Ajout":dat,"Responsable":"su"})
            x = global_col_apps.find_one({"Nom_App":"RDConnect"})
            total_old = int(str(x["Total_patients"]))
            total_old = total_old+1
            global_col_apps.update_one({"Nom_App":"RDConnect"},{"$set":{"Total_patients":str(total_old)}})
        ...
        return jsonify({"msg":"failure","urlto":"/loginUsers?initApp=RDConnect"})

def RDconnectlistPat():
    data = request.get_json(force=True)
    resp = str(data["for"])
    if session.get(f"{resp},RDConnect"):
        y=""
...
def RDconnectDelPat():
    ...
    if session.get(f"{deleter},RDConnect"):
        if global_col_patients.count_documents({"ID":targDel}) >= 1:
            global_col_patients.delete_many({"ID":targDel})
            RD_vol_infx.delete_many({"ID":targDel})
            RD_vol_poum.delete_many({"ID":targDel})
            x = global_col_apps.find_one({"Nom_App":"RDConnect"})
            ...
            global_col_apps.update_one({"Nom_App":"RDConnect"},{"$set":{"Total_patients":str(nexTotP)}})
...
def RDconnectreturnImageOriginal(pat,usr):
    data = request.get_json(force=True)
    mode = int(data["mode"])
...
def RDconnectreturnImageLesion(pat,usr):
    data = request.get_json(force=True)
    mode = int(data["mode"])
    indx = str(data["imgIndex"])
...
def RDconnectreturnImageOrgane(pat,usr):
    data = request.get_json(force=True)
    mode = int(data["mode"])
    indx = str(data["imgIndex"])
...
def RDconnectimgNone():
	path_to_img = "./aucune.png"
	return send_file(path_to_img)

def RDconnectreplaceImage(pat,usr):
    data = request.get_json(force=True)
    mode = int(data["mode"])
...
def RDconnectcountTotal(pat,usr):
    data = request.get_json(force = True)
    mode = int(data["mode"])
...
def RDconnectdeleteOne(pat,usr):
    data = request.get_json(force = True)
    mode = int(data["mode"])
...
def RDconnectdeleteMany(pat,usr):
    data = request.get_json(force = True)
    mode = int(data["mode"])
...
def RDconnectConstruction3d(pat,usr):
...
def RDconnectDownload_3d_file(pat,usr):
	return send_file("./upload/"+pat+"/3dfile/model3D.obj")

def RDconnectDownload_3d_file2(pat,usr):
...
def Reconstruction3D_Lesion(userdirectory):
	...
	ginfx = RD_vol_infx.count_documents({"ID":str(userdirectory)})
	if (ginfx == 0):
		RD_vol_infx.insert_one({"ID":str(userdirectory),"volINFX":str(volINFX)})
...
def Reconstruction3D_Poumons(userdirectory):
	...
	gpoum = RD_vol_poum.count_documents({"ID":str(userdirectory)})
	if (gpoum == 0):
		RD_vol_poum.insert_one({"ID":str(userdirectory),"volPOUM":str(volPOUM)})

def RDconnecttauxInfection(pat,usr):
	data = request.get_json(force = True)
	targetUser = str(data["requested"])
	tmpInfx = RD_vol_infx.find_one({"ID":pat})
	tmpPoum = RD_vol_poum.find_one({"ID":pat})
...
#########################################################################################
#########################################################################################
##def rdconnectPred(imgx):
##    model = tf.keras.models.load_model('./models/rddmla.h5',compile = False)
##    img = image.load_img(imgx,target_size=(224,224))
##    print(imgx)
##    x=np.array(img)
##    print("///////////////"+imgx)
##    x=x/255
...
##    print(preds)
##    if preds==0:
##        return "Wet Age-related Macular Degeneration Image"
##    elif preds==1:
##        return "HEALTHY Image"
##    elif preds==2:
##        return "Proliferative Diabetic Retinopathy Image"
##
##
##def RDconnectClassifier(pat,usr):
##    y=""
##    path="./upload/"+pat+"/original"+"/*.png"
##    for filename in natsorted(glob.glob(path)):
##        y=y+f"{filename}"+"&&"+f"{rdconnectPred(filename)}"+"///"
##    return jsonify({"msg":"success","ls":f"{y}"})

def RDconnectClassifier(pat,usr):
    model = tf.keras.models.load_model('./models/rddmla.h5',compile = False)
    test_generator = ImageDataGenerator(preprocessing_function=preprocess_input) # VGG16 preprocessing
    testgen = test_generator.flow_from_directory("./upload/"+pat,target_size=(224, 224),class_mode=None,batch_size=1,shuffle=False,)
    preds = model.predict(testgen)
    preds=np.argmax(preds, axis=1)
    ffrd = str(preds).replace("[","").replace("]","")
    return jsonify({"msg":"success","ls":f"{ffrd}"})