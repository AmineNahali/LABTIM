...
function putUserDetails() {
	fetch("/app/COVConnect/covconnectUserDetails",{method:'POST',body:JSON.stringify({tr:targetUserMail.toString()}),contentType:'application/json'})
	.then(res=>res.json())
	.then((data)=>{
		if (data.msg == "success") {
...
function logout(){
	fetch("/app/COVConnect/covconnectLogOut",{method:'POST',body:JSON.stringify({tr:targetUserMail.toString()}),contentType:'application/json'})
	.then(res=>res.json())
	.then((data)=>{
		if (data.msg == "success"){
...
async function uploadFile(f) {
	...
	form.append('file', f);	
	fetch("/app/COVConnect/upload/"+targetPatientUpload, { method: 'POST', body:form })
	...
	.then((data)=>{
		if (data.msg == "success")
		{
			...
			if(uploadedSuccessfully==totalFilesToUpload)
			{
				...
				fetch("/app/COVConnect/generateThumbnails/"+targetPatientUpload,{method : 'POST'})
				.then(res => res.json())
				.then((data) =>{
					if(data.msg == "success"){
...
function avantUpl(){
    ...
    if(document.getElementById('inputPatientID').value.toString()==""){alert("une ID patient est nécessaire");}
        ...
        fetch("/app/COVConnect/covconnectAddPat",{method:'POST',body:JSON.stringify(snd),contentType:'application/json'})
        .then(res=>res.json())
        .then((data)=>{
            if(data.msg=="success"){
...
function loadPatients(){
    ...
    fetch("/app/COVConnect/listPat",{method:'POST',body:JSON.stringify({for:targetUserMail.toString()}),contentType:'application/json'})
    .then(res=>res.json())
    .then((data)=>{
        if(data.msg=="success"){
...
function confirmDeletingPat(){
    fetch("/app/COVConnect/covconnectDelPat",{method:'POST',body:JSON.stringify({targDel:targetPat.toString(),deleter:targetUserMail}),contentType:'application/json'})
    .then(res=>res.json())
    .then((data)=>{
        if(data.msg == 'success'){
            loadPatients();
...
function putImage(x){
	fetch("/app/COVConnect/returnImageOriginal/"+targetPat.toString()+"/"+targetUserMail.toString(),{method: 'POST',body:JSON.stringify({mode:modeUtilisation.toString(),imgIndex : x.toString()}),contentType : "application/json"})
	.then((data) => data.blob())
	.then((imgFileOriginal)=>{document.getElementById('outputImagePreviewOriginal').src = URL.createObjectURL(imgFileOriginal);
        fetch("/app/COVConnect/returnImageLesion/"+targetPat.toString()+"/"+targetUserMail.toString(),{method: 'POST',body:JSON.stringify({mode:modeUtilisation.toString(),imgIndex : x.toString()}),contentType : "application/json"})
        .then((data) => data.blob())
        .then((imgFileLesion)=>{
            document.getElementById('outputImagePreviewLesion').src = URL.createObjectURL(imgFileLesion);
            fetch("/app/COVConnect/returnImageOrgane/"+targetPat.toString()+"/"+targetUserMail.toString(),{method: 'POST',body:JSON.stringify({mode:modeUtilisation.toString(),imgIndex : x.toString()}),contentType : "application/json"})
            .then((data) => data.blob())
            .then((imgFileOrgane)=>{
...
function putNone(){
	fetch('/app/COVConnect/returnImageNone',{method: 'POST'})
	.then((data) => data.blob())
	.then((imgFile)=>{
	...
	if (input_a_remplacer.value.toString() == "" || input_a_remplacer.value.toString().split('').includes('-') || input_remplacer_par.value.toString() == "" || input_remplacer_par.value.toString().split('').includes('-')){alert('données incorrectes');}
		...
		if (document.getElementById('finindicedimage').innerHTML== '0') {alert("pas d'images pour faire un remplacement");}
			...
			if(parseInt(input_a_remplacer.value.toString()) > parseInt(document.getElementById('finindicedimage').innerHTML) || parseInt(input_remplacer_par.value.toString()) > parseInt(document.getElementById('finindicedimage').innerHTML)){alert('données incorrectes');}
				...
				fetch("/app/COVConnect/replaceImage/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'POST',body:JSON.stringify({mode:modeUtilisation.toString(),indx_a_remplacer:input_a_remplacer.value.toString(),indx_remplacer_par:input_remplacer_par.value.toString()}),contentType : "application/json"})
				.then((res)=> res.json())
				.then((data)=>{
				if (data.msg == 'success') {
					setIndex();
	...
	if(input_supprimer.value.toString() == "" || input_supprimer.value.toString().split('').includes('-')){alert('données incorrectes');}
		...
		if (document.getElementById('finindicedimage').innerHTML == '0') {alert("pas d'images pour faire une suppression");}
			...
			if (parseInt(input_supprimer.value.toString()) > parseInt(document.getElementById('finindicedimage').innerHTML)){alert('données incorrectes');}
				...
				fetch("/app/COVConnect/deleteOne/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'POST',body:JSON.stringify({mode:modeUtilisation.toString(),deleted:input_supprimer.value.toString()}),contentType : "application/json"})
				.then((res)=> res.json())
				.then((data)=>{
				if (data.msg == 'success') {
					updateTotal();
	...
	if (input_supprimer_plusieurs_gauche.value.toString() == "" || input_supprimer_plusieurs_droite.value.toString() == "" || input_supprimer_plusieurs_gauche.value.toString().split('').includes('-') || input_supprimer_plusieurs_droite.value.toString().split('').includes('-')) {alert('données incorrectes');}
		...
		if (document.getElementById('finindicedimage').innerHTML == '0') {alert("pas d'images pour faire une suppression");}
			...
			if(parseInt(input_supprimer_plusieurs_gauche.value.toString()) > parseInt(document.getElementById('finindicedimage').innerHTML) || parseInt(input_supprimer_plusieurs_droite.value.toString()) > parseInt(document.getElementById('finindicedimage').innerHTML)){alert('données incorrectes');}
				...
				{fetch("/app/COVConnect/deleteMany/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'POST',body:JSON.stringify({mode:modeUtilisation.toString(),left:input_supprimer_plusieurs_gauche.value.toString(),right:input_supprimer_plusieurs_droite.value.toString()}),contentType : "application/json"})
				.then((res)=> res.json())
				.then((data)=>{
					if (data.msg == 'success') {
						updateTotal();
...
function updateTotal(){
	...
	fetch("/app/COVConnect/returnTotalImages/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'POST',body:JSON.stringify({mode:modeUtilisation.toString()}),contentType:"application/json"})
		.then((res)=> res.json())
		.then((data)=>{
...
document.getElementById('btnSegmentation').addEventListener('click',(e)=>{
	...
	document.getElementById('loadz').style.display = 'block';
	fetch("/app/COVConnect/segmenter/",{method:'POST',body:JSON.stringify({pat:targetPat.toString(),usr:targetUserMail.toString()}),contentType:'application/json'})
	.then((res)=>res.json())
	.then((data)=>{
		if(data.msg == 'success'){
...
document.getElementById('btnReconstruction3d').addEventListener('click',(e)=>{
	...
	fetch("/app/COVConnect/construction3d/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'POST'})
	.then((res)=>res.json())
	.then((data)=>{
		if(data.msg == 'success'){
			document.getElementById('msgloadz').innerHTML = "Chargement du modèle 3D 'Lésion', veuillez patienter";
			fetch("/app/COVConnect/download3dfile/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'GET'})
			.then(res=>res.blob())
			.then(blob100 => {
				...
				fetch("/app/COVConnect/download3dfile2/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'GET'})
				.then(res=>res.blob())
				.then(blob200=>{
...
document.getElementById('btnSegmRecons').addEventListener('click',(e)=>{
	...
	document.getElementById('msgloadz').innerHTML = "Segmentation en cours, veuillez patienter";
	document.getElementById('loadz').style.display = 'block';
	fetch("/app/COVConnect/segmenter/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'POST'})
	.then((res)=>res.json())
	.then((data)=>{
		if(data.msg == 'success'){
			document.getElementById('btnReconstruction3d').disabled = false;
...
function setTauxInfection(ssinfx){
	fetch("/app/COVConnect/getInfectionRate/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'POST',body:JSON.stringify({requested:ssinfx.toString()}),contentType:'application/json'})
	.then(res=>res.json())
	.then((data)=>{
