...
function putUserDetails() {
	fetch("/app/mTLEConnect/mTLEConnectUserDetails",{method:'POST',body:JSON.stringify({tr:targetUserMail.toString()}),contentType:'application/json'})
	.then(res=>res.json())
	.then((data)=>{
		if (data.msg == "success") {
...
function logout(){
	fetch("/app/mTLEConnect/mTLEConnectLogOut",{method:'POST',body:JSON.stringify({tr:targetUserMail.toString()}),contentType:'application/json'})
	.then(res=>res.json())
	.then((data)=>{
		if (data.msg == "success"){
...
async function uploadFile(f) {
	...
	form.append('file', f);	
	fetch("/app/mTLEConnect/upload/"+targetPatientUpload, { method: 'POST', body:form })
	...
	.then((data)=>{
		if (data.msg == "success")
		{
			...
			if(uploadedSuccessfully==totalFilesToUpload)
			{
				...
				fetch("/app/mTLEConnect/generateThumbnails/"+targetPatientUpload,{method : 'POST'})
				.then(res => res.json())
				.then((data) =>{
					if(data.msg == "success"){
...
function avantUpl(){
    ...
    if(document.getElementById('inputPatientID').value.toString()==""){alert("une ID patient est nécessaire");}
        ...
        fetch("/app/mTLEConnect/mTLEConnectAddPat",{method:'POST',body:JSON.stringify(snd),contentType:'application/json'})
        .then(res=>res.json())
        .then((data)=>{
            if(data.msg=="success"){
...
function loadPatients(){
    ...
    fetch("/app/mTLEConnect/listPat",{method:'POST',body:JSON.stringify({for:targetUserMail.toString()}),contentType:'application/json'})
    .then(res=>res.json())
    .then((data)=>{
        if(data.msg=="success"){
...
function confirmDeletingPat(){
    fetch("/app/mTLEConnect/mTLEConnectDelPat",{method:'POST',body:JSON.stringify({targDel:targetPat.toString(),deleter:targetUserMail}),contentType:'application/json'})
    .then(res=>res.json())
    .then((data)=>{
        if(data.msg == 'success'){
            loadPatients();
...
function putImage(x){
	fetch("/app/mTLEConnect/returnImageOriginal/"+targetPat.toString()+"/"+targetUserMail.toString(),{method: 'POST',body:JSON.stringify({mode:modeUtilisation.toString(),imgIndex : x.toString()}),contentType : "application/json"})
	.then((data) => data.blob())
	.then((imgFileOriginal)=>{document.getElementById('outputImagePreviewOriginal').src = URL.createObjectURL(imgFileOriginal);
        fetch("/app/mTLEConnect/returnImageLesion/"+targetPat.toString()+"/"+targetUserMail.toString(),{method: 'POST',body:JSON.stringify({mode:modeUtilisation.toString(),imgIndex : x.toString()}),contentType : "application/json"})
        .then((data) => data.blob())
        .then((imgFileLesion)=>{
            document.getElementById('outputImagePreviewLesion').src = URL.createObjectURL(imgFileLesion);
            fetch("/app/mTLEConnect/returnImageOrgane/"+targetPat.toString()+"/"+targetUserMail.toString(),{method: 'POST',body:JSON.stringify({mode:modeUtilisation.toString(),imgIndex : x.toString()}),contentType : "application/json"})
            .then((data) => data.blob())
            .then((imgFileOrgane)=>{
...
function putNone(){
	fetch('/app/mTLEConnect/returnImageNone',{method: 'POST'})
	.then((data) => data.blob())
	.then((imgFile)=>{
	...
	if (input_a_remplacer.value.toString() == "" || input_a_remplacer.value.toString().split('').includes('-') || input_remplacer_par.value.toString() == "" || input_remplacer_par.value.toString().split('').includes('-')){alert('données incorrectes');}
		...
		if (document.getElementById('finindicedimage').innerHTML== '0') {alert("pas d'images pour faire un remplacement");}
			...
			if(parseInt(input_a_remplacer.value.toString()) > parseInt(document.getElementById('finindicedimage').innerHTML) || parseInt(input_remplacer_par.value.toString()) > parseInt(document.getElementById('finindicedimage').innerHTML)){alert('données incorrectes');}
				...
				fetch("/app/mTLEConnect/replaceImage/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'POST',body:JSON.stringify({mode:modeUtilisation.toString(),indx_a_remplacer:input_a_remplacer.value.toString(),indx_remplacer_par:input_remplacer_par.value.toString()}),contentType : "application/json"})
				.then((res)=> res.json())
				.then((data)=>{
				if (data.msg == 'success') {
					setIndex();
	...
	if(input_supprimer.value.toString() == "" || input_supprimer.value.toString().split('').includes('-')){alert('données incorrectes');}
		...
		if (document.getElementById('finindicedimage').innerHTML == '0') {alert("pas d'images pour faire une suppression");}
			...
			if (parseInt(input_supprimer.value.toString()) > parseInt(document.getElementById('finindicedimage').innerHTML)){alert('données incorrectes');}
				...
				fetch("/app/mTLEConnect/deleteOne/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'POST',body:JSON.stringify({mode:modeUtilisation.toString(),deleted:input_supprimer.value.toString()}),contentType : "application/json"})
				.then((res)=> res.json())
				.then((data)=>{
				if (data.msg == 'success') {
					updateTotal();
	...
	if (input_supprimer_plusieurs_gauche.value.toString() == "" || input_supprimer_plusieurs_droite.value.toString() == "" || input_supprimer_plusieurs_gauche.value.toString().split('').includes('-') || input_supprimer_plusieurs_droite.value.toString().split('').includes('-')) {alert('données incorrectes');}
		...
		if (document.getElementById('finindicedimage').innerHTML == '0') {alert("pas d'images pour faire une suppression");}
			...
			if(parseInt(input_supprimer_plusieurs_gauche.value.toString()) > parseInt(document.getElementById('finindicedimage').innerHTML) || parseInt(input_supprimer_plusieurs_droite.value.toString()) > parseInt(document.getElementById('finindicedimage').innerHTML)){alert('données incorrectes');}
				...
				{fetch("/app/mTLEConnect/deleteMany/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'POST',body:JSON.stringify({mode:modeUtilisation.toString(),left:input_supprimer_plusieurs_gauche.value.toString(),right:input_supprimer_plusieurs_droite.value.toString()}),contentType : "application/json"})
				.then((res)=> res.json())
				.then((data)=>{
					if (data.msg == 'success') {
						updateTotal();
...
function updateTotal(){
	...
	fetch("/app/mTLEConnect/returnTotalImages/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'POST',body:JSON.stringify({mode:modeUtilisation.toString()}),contentType:"application/json"})
		.then((res)=> res.json())
		.then((data)=>{
...
document.getElementById('btnSegmentation').addEventListener('click',(e)=>{
	...
	document.getElementById('loadz').style.display = 'block';
	fetch("/app/mTLEConnect/segmenter/",{method:'POST',body:JSON.stringify({pat:targetPat.toString(),usr:targetUserMail.toString()}),contentType:'application/json'})
	.then((res)=>res.json())
	.then((data)=>{
		if(data.msg == 'success'){
...
document.getElementById('btnReconstruction3d').addEventListener('click',(e)=>{
	...
	fetch("/app/mTLEConnect/construction3d/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'POST'})
	.then((res)=>res.json())
	.then((data)=>{
		if(data.msg == 'success'){
			document.getElementById('msgloadz').innerHTML = "Chargement du modèle 3D 'Lésion', veuillez patienter";
			fetch("/app/mTLEConnect/download3dfile/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'GET'})
			.then(res=>res.blob())
			.then(blob100 => {
				...
				fetch("/app/mTLEConnect/download3dfile2/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'GET'})
				.then(res=>res.blob())
				.then(blob200=>{
...
document.getElementById('btnSegmRecons').addEventListener('click',(e)=>{
	...
	document.getElementById('msgloadz').innerHTML = "Segmentation en cours, veuillez patienter";
	document.getElementById('loadz').style.display = 'block';
	fetch("/app/mTLEConnect/segmenter/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'POST'})
	.then((res)=>res.json())
	.then((data)=>{
		if(data.msg == 'success'){
			document.getElementById('btnReconstruction3d').disabled = false;
...
function setTauxInfection(ssinfx){
	fetch("/app/mTLEConnect/getInfectionRate/"+targetPat.toString()+"/"+targetUserMail.toString(),{method:'POST',body:JSON.stringify({requested:ssinfx.toString()}),contentType:'application/json'})
	.then(res=>res.json())
	.then((data)=>{
